...
describe('Service: GlobalErrorHandler', () => {
    ...
    TestBed.configureTestingModule({
      imports: [
        RouterTestingModule.withRoutes([
          {
            path: '',
            component: NotFoundComponent, // any component just for a mock up
          },
        ]),
        NotFoundModule,
      ],
  ...
  it('should log error via errorsService', inject([
    ...
    spyOn(errorsService, 'log').and.callThrough();

    service.handleError(MOCK_ERROR_RESPONSE);

    expect(errorsService.log).toHaveBeenCalled();
    expect(errorsService.log).toHaveBeenCalledWith(MOCK_ERROR_RESPONSE);
  }));
});
